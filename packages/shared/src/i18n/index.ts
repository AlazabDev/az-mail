import {DEFAULT_LANGUAGE, isValidLanguageCode, type Language} from './languages.js';
...
export async function loadTranslations(languageCode: string): Promise<Translations> {
  ...
  // Fallback to the configured default language if language not found
  console.warn(
    `Translation file not found for ${languageCode}, falling back to ${DEFAULT_LANGUAGE}`,
  );
...
export function loadTranslationsSync(languageCode: string): Translations {
  ...
  // Fallback to the configured default language if language not found
  if (languageCode !== DEFAULT_LANGUAGE) {
    return loadTranslationsSync(DEFAULT_LANGUAGE);
  }
...
export async function createTranslator(languageCode: string): Promise<Translator> {
  const resolvedLanguageCode = isValidLanguageCode(languageCode) ? languageCode : DEFAULT_LANGUAGE;
  const translations = await loadTranslations(resolvedLanguageCode);
  return new Translator(translations, resolvedLanguageCode);
}
...
export function createTranslatorSync(languageCode: string): Translator {
  const resolvedLanguageCode = isValidLanguageCode(languageCode) ? languageCode : DEFAULT_LANGUAGE;
  const translations = loadTranslationsSync(resolvedLanguageCode);
  return new Translator(translations, resolvedLanguageCode);
}
